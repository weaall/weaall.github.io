...
export default function NewPage({ collapsed, docId, categories = [] }: { collapsed: boolean; docId: string; categories?: string[] }) {
    ...
    const handlePlusClick = (id: string) => {
        ...
        setTimeout(() => {
            ...
            if (btn) {
                ...
                const menuWidth = 265;
                const menuHeight = 220; // 메뉴 자체 높이(드로워는 TypeMenu에서 위/아래로 알아서 펼침)
                const margin = 12;
                let left = rect.left - menuWidth - 45;
                let top = rect.top - 70;
                // 화면 밖으로 넘어가지 않도록 클램핑(버튼 옆에 붙은 위치 유지)
    ...
    return (
        <tw.Container
            ...
            className="m:pl-0! m:py-8 m:overflow-x-hidden"
            style={{
                paddingLeft: collapsed ? 50 : 350,
                transition: "padding-left 0.2s",
            }}
        >
            <div data-editor-col className="max-w-[712px] min-w-[712px] w-[712px] mx-10 m:min-w-0 m:w-full m:max-w-full m:mx-0 m:px-5 m:overflow-x-hidden" style={{ position: "relative" }}>
                <tw.BlockWrap>
