...
export const Container = tw.div`flex px-10 relative border-r border-white/[0.45] tracking-tight`;
...
export const IconBtn = tw.button`p-5px flex items-center justify-center rounded-md hover:bg-white/10 transition-colors`;
...
export const CategoryButton = tw.button`
  ...
  text-[#aaa] hover:bg-white/[0.08] max-w-[244px]
`;
export const CategoryList = tw.ul``;
export const CategoryItem = tw.li`mb-[1px]`;
export const PostLink = tw(Link)<{ $active?: boolean }>`
  flex items-center max-w-[244px] px-2 py-5px rounded-md hover:bg-white/[0.1] font-medium overflow-hidden text-white/50
  ${({ $active }) => ($active ? "bg-white/[0.1] text-white" : "")}
`;

export const SvgWrap = tw.div`w-5 h-5 flex-shrink-0 mr-2 flex items-center justify-center text-white/50`;
